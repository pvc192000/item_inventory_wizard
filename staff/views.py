...
def item_orders(request):
    if request.method == "POST":
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT quantity FROM item where item_id = {};" .format(i_id))
        items = dictfetchall(cursor)
        original_qty = 0
        original_qty = items[0]['quantity']
        cursor.execute( "UPDATE item SET quantity = {} WHERE item_id = {};".format(original_qty - qty, i_id))
        cursor = connection.cursor()
        cursor.execute("Delete FROM purchase where customer_id = {} and item_id = {} and quantity = {}" .format(c_id, i_id, qty))
        cursor = connection.cursor()
...
def delivered(request):
    if request.method == "POST":
        ...
        cursor.execute( "UPDATE item SET quantity = {} WHERE item_id = {};".format(original_qty + qty, i_id))
        cursor = connection.cursor()
        cursor.execute("Delete FROM order_from where order_id = {}" .format(o_id))
        cursor = connection.cursor()
